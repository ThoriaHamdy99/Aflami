...
class UserListRemoteDataSourceImplTest {
    ...
    fun `getMoviesFromList should return a list of movies and call the api service`() = runTest {
        val listId = 1L
        coEvery { userListApiService.getMoviesAndShowsFromList(listId, 1) } returns remoteListResponse

        val result = userListRemoteDataSourceImpl.getMoviesAndShowsFromList(listId, 1)

        assertThat(result).isEqualTo(remoteListResponse)
        coVerify(exactly = 1) { userListApiService.getMoviesAndShowsFromList(listId, 1) }
    }

    @Test
    fun `getMoviesFromList should throw NetworkException when api call fails`() = runTest {
        val listId = 1L
        coEvery { userListApiService.getMoviesAndShowsFromList(listId, 1) } throws NetworkException()

        assertThrows<NetworkException> { userListRemoteDataSourceImpl.getMoviesAndShowsFromList(listId, 1) }
    }
