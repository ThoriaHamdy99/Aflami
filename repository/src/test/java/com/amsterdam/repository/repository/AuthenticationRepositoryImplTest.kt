...
class AuthenticationRepositoryImplTest {
    ...
    @Test
    fun `loginWithPassword should not call cacheSessionId or setSessionType when login fails`() =
        runTest {
            coEvery {
                authenticationRemoteDataSource.loginWithPassword(
                    any(),
    ...
    @Test
    fun `logout should should not call setSessionType or deleteAccountDetails when clearing session fails`() =
        runTest {
            coEvery { authenticationLocalDataSource.clearCachedSessionId() } throws RuntimeException()
            assertThrows<RuntimeException> {
                repository.logout()
