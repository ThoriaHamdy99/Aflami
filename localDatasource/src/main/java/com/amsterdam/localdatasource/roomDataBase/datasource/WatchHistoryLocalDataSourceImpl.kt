...
class WatchHistoryLocalDataSourceImpl @Inject constructor(
    ...
    override suspend fun upsertMovieToWatchHistory(item: MovieWatchHistoryDto) = dao.addMovieToWatchHistory(item)
    ...
    override suspend fun upsertTvShowToWatchHistory(item: TvShowWatchHistoryDto) = dao.addTvShowToWatchHistory(item)
